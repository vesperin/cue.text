...
class TextFailover {
  ...
  static final List<String> DEFAULT_TEXT = ImmutableList.of(
    "Google Java Style",
    ...
    "2.3.2 Special escape sequences",
    "For any character that has a special escape sequence, that sequence is used rather than the corresponding octal (e.g. \\012) or Unicode (e.g. \\u000a) escape.",
    "2.3.3 Non-ASCII characters",
    ...
    "Exception: A single parameter-less annotation may instead appear together with the first line of the signature, for example:",
    ...
    "static final ImmutableList<String> NAMES = ImmutableList.of();",
    ...
    "Convert the phrase to plain ASCII and remove any apostrophes. For example, \"Muller algorithm\" might become \"Muellers algorithm\".",
    ...
    "// do stuff",
      ...
      "So, now I just want to write some extremely simple code that will make sure that my Java runtime environment can link to these Guice JARs. (Say that five times fast.). I am assuming you can afford one of those fancy-dancy IDE tools like NetBeans or Eclipse, and that you have some knowledge of configuring a project and a classpath in those expensive development tools of yours. All I can afford is the JDK, which is installed in my _jdk1.6 directory off the root of C:\\. I’m hoping that if I can get Guice to work using nothing more than a command prompt, Emacs and a JDK, that getting this working in a marvelous IDE should be a lead pipe cinch for you. To code the old-school way, I have created a subset of folders off my C:\\ drive that looks like this:",
      "Getting Started with Google Guice",
      ...
      "Injector[bindings=[Key[type=com.google.inject.Injector, annotation=[none]]=BindingImpl[key=Key[type=com.google.inject.Injector, annotation=[none]], source=com.google.inject.Guice.createInjector(Guice.java:75), filter=Provider<Injector>],",
      "Key[type=java.util.logging.Logger, annotation=[none]]=BindingImpl[key=Key[type=java.util.logging.Logger, annotation=[none]], source=com.google.inject.Guice.createInjector(Guice.java:75), filter=Provider<Logger>], Key[type=com.google.inject.Stage, annotation=[none]]=BindingImpl[key=Key[type=com.google.inject.Stage, annotation=[none]], source=com.google.inject.Guice.createInjector(Guice.java:75),filter=ConstantFactory[value=DEVELOPMENT]]]]",
